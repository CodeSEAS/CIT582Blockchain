...
def send_tokens_algo( acl, sender_sk, txes):
    ...
    # return []
    r_pk = txes[0]['receiver_pk']
    t_am = txes[0]['send_amount']
    unsigned_tx = transaction.PaymentTxn(sender_pk, params, r_pk, t_am)
    s_tx = unsigned_tx.sign(sender_sk)
    sent = acl.send_transaction(s_tx)
    wait_for_confirmation_algo(acl, sent)
    return sent
...
def send_tokens_eth(w3,sender_sk,txes):
    ...
    s = w3.eth.get_transaction_count(sender_pk, 'pending')
    r_pk = txes[0]['receiver_pk']
    s_am = txes[0]['send_amount']

    tx_dict = {
        'nonce': s,
        'gasPrice': w3.eth.gas_price,
        'gas': w3.eth.estimate_gas({'from': sender_pk, 'to': r_pk, 'data': b'', 'amount': s_am}),
        'to': r_pk,
        'value': s_am,
        'data': b''}
